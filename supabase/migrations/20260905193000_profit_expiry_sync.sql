-- Keep expiry warehouse off sale invoices / sale returns / purchase returns.
-- Profit follows posted ledgers:
--   customer EXR credits reduce net sales (not mixed into sale_returns)
--   vendor CLM debits minus SET rejects recover cost (not mixed into purchase_returns)
-- COGS still uses saleable stock only (expiry does not restore warehouse stock).
...
declare
  v_sales numeric := 0;
  v_returns numeric := 0;
  v_expiry_credits numeric := 0;
  v_expiry_claims numeric := 0;
  v_expiry_rejects numeric := 0;
  v_expiry_vendor_net numeric := 0;
  v_cogs numeric := 0;
...
begin
  ...
  select coalesce(sum(grand_total), 0) into v_expiry_credits
  from public.expiry_receipts
  where company_id = p_company_id
    and status = 'posted'
    and receipt_date between p_from and p_to;

  select coalesce(sum(grand_total), 0) into v_expiry_claims
  from public.expiry_claims
  where company_id = p_company_id
    and status = 'posted'
    and claim_date between p_from and p_to;

  select coalesce(sum(rejected_amount), 0) into v_expiry_rejects
  from public.expiry_settlements
  where company_id = p_company_id
    and settlement_date between p_from and p_to;

  v_expiry_vendor_net := v_expiry_claims - v_expiry_rejects;
  ...
  v_net_sales := v_sales - v_returns - v_expiry_credits;
  v_cogs_net := v_cogs - v_cogs_ret;
  v_gross := v_net_sales - v_cogs_net;
  v_net_profit := v_gross + v_expiry_vendor_net - v_expenses;
  ...
  from (
    select
      ...
      coalesce(x.expiry_credits, 0) as expiry_credits,
      coalesce(c.expiry_claims, 0) - coalesce(j.expiry_rejects, 0) as expiry_vendor,
      coalesce(s.sales, 0) - coalesce(r.returns, 0) - coalesce(x.expiry_credits, 0) as net_sales,
    ...
    left join (
      select receipt_date as day, sum(grand_total) as expiry_credits
      from public.expiry_receipts
      where company_id = p_company_id and status = 'posted'
        and receipt_date between p_from and p_to
      group by 1
    ...
    left join (
      select claim_date as day, sum(grand_total) as expiry_claims
      from public.expiry_claims
      where company_id = p_company_id and status = 'posted'
        and claim_date between p_from and p_to
      group by 1
    ...
    left join (
      select settlement_date as day, sum(rejected_amount) as expiry_rejects
      from public.expiry_settlements
      where company_id = p_company_id
        and settlement_date between p_from and p_to
      group by 1
  ...
  return jsonb_build_object(
    ...
    'sales', v_sales,
    'returns', v_returns,
    'expiry_credits', v_expiry_credits,
    'expiry_claims', v_expiry_claims,
    'expiry_rejects', v_expiry_rejects,
    'expiry_vendor_net', v_expiry_vendor_net,
    ...
    'net_margin_pct', case when v_net_sales > 0 then round((v_net_profit / v_net_sales) * 100, 1) else 0 end,
