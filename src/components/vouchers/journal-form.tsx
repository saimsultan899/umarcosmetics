  ...
  async function onSubmit(e: FormEvent) {
    ...
    const valid = lines.filter(
      (l) =>
        l.debit_party_id &&
        l.credit_party_id &&
        l.debit_party_id !== l.credit_party_id &&
        Number(l.amount) > 0,
    );
    if (valid.length === 0) {
      setError("Add balanced JV lines with debit, credit, and amount.");
      return;
    }
              ...
              <tr key={line.key} className="border-t border-[var(--border)]">
                <td className="px-3 py-2">
                  <PartyCodeCell
                    companyId={companyId}
                    parties={parties}
                    value={line.debit_party_id}
                    onChange={(partyId) =>
                      setLines((prev) =>
                        prev.map((l) =>
                          l.key === line.key
                            ? { ...l, debit_party_id: partyId }
                            : l,
                        ),
                      )
                    }
                ...
                <td className="px-3 py-2">
                  <PartyCodeCell
                    companyId={companyId}
                    parties={parties}
                    value={line.credit_party_id}
                    onChange={(partyId) =>
                      setLines((prev) =>
                        prev.map((l) =>
                          l.key === line.key
                            ? { ...l, credit_party_id: partyId }
                            : l,
                        ),
                      )
                    }
